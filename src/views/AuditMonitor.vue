        ...
        <div class="echart-box">
          ...
          <el-card style="position: relative; top: 2px">
            ...
            <div class="release-box">
              <el-text
                v-if="releaseStatus.complete"
                type="success"
                class="info-releaseStatus"
              >
                放车已完成
              </el-text>
          ...
          <el-card class="passcheck-box">
            ...
            <div class="table-box">
              <el-table :data="tableData" stripe>
                <el-table-column prop="NOTE" label="道闸名称" width="250" />
                <el-table-column prop="VALUE" label="有效期" width="130" />
...
const taxiDataComputed = computed(() => {
  ...
  return {
    ...
    onWayTotal:
      ...
      taxiData.onRoad.T1.short +
      taxiData.onRoad.T2.long +
      taxiData.onRoad.T2.short,
  };
});
...
watch(taxiData, (taxiData) => {
  ...
  optionTermnl.series[0].data[0].value = taxiData.terminal.T1.long.line1;
  optionTermnl.series[0].data[1].value = taxiData.terminal.T1.long.line2;
  optionTermnl.series[0].data[2].value = taxiData.terminal.T1.long.line3;
  ...
  optionTermnl.series[0].data[5].value = taxiData.terminal.T2.short.line1;
  update();
});
...
async function init() {
  ...
  await axios.get("/api/get_taxi_num").then((res) => {
    ...
    for (let i = 0; i < data.length; i++) {
      switch (data[i].ID) {
        case "poolT1":
          taxiData.pool.T1.num = data[i].NUM;
          break;
        case "poolT2":
          taxiData.pool.T2.num = data[i].NUM;
          break;
        case "wayT1long":
          taxiData.onRoad.T1.long = data[i].NUM;
          break;
        case "wayT2long":
          taxiData.onRoad.T2.long = data[i].NUM;
          break;
        case "wayT2short":
          taxiData.onRoad.T2.short = data[i].NUM;
          break;
        case "terminalT1long1":
          taxiData.terminal.T1.long.line1 = data[i].NUM;
          break;
        case "terminalT1long2":
          taxiData.terminal.T1.long.line2 = data[i].NUM;
          break;
        case "terminalT1long3":
          taxiData.terminal.T1.long.line3 = data[i].NUM;
          break;
        case "terminalT2long1":
          taxiData.terminal.T2.long.line1 = data[i].NUM;
          break;
        case "terminalT2long2":
          taxiData.terminal.T2.long.line2 = data[i].NUM;
          break;
        case "terminalT2short1":
          taxiData.terminal.T2.short.line1 = data[i].NUM;
          break;
        default:
          break;
      }
...
function switchData(type, mark, data) {
  console.log("switchData", type, mark, data);
  switch (type) {
    case "warning":
      switch (mark) {
        case "confirm":
      ...
      switch (mark) {
        case "status_num":
          data.forEach((item) => {
            switch (item.location) {
              case "pool":
                if (item.terminal == "T1") {
                  taxiData.pool.T1.num = item.num;
                } else {
                  taxiData.pool.T2.num = item.num;
                }
                break;
              case "way":
                if (item.terminal == "T1" && item.type == "long") {
                  taxiData.onRoad.T1.long = item.num;
                } else if (item.terminal == "T2" && item.type == "long") {
                  taxiData.onRoad.T2.long = item.num;
                } else {
                  taxiData.onRoad.T2.short = item.num;
                }
                break;
              case "terminal":
                if (
                  ...
                  taxiData.terminal.T1.long.line1 = item.num;
                } else if (
                  item.terminal == "T1" &&
                  item.type == "long" &&
                  item.line == 2
                ) {
                  taxiData.terminal.T1.long.line2 = item.num;
                } else if (
                  item.terminal == "T1" &&
                  item.type == "long" &&
                  item.line == 3
                ) {
                  taxiData.terminal.T1.long.line3 = item.num;
                } else if (
                  item.terminal == "T2" &&
                  item.type == "long" &&
                  item.line == 1
                ) {
                  taxiData.terminal.T2.long.line1 = item.num;
                } else if (
                  item.terminal == "T2" &&
                  item.type == "long" &&
                  item.line == 2
                ) {
                  taxiData.terminal.T2.long.line2 = item.num;
                } else {
                  taxiData.terminal.T2.short.line1 = item.num;
                }
                break;
              default:
                break;
        ...
        case "pool_out_countdown":
          if (data.num) {
            ...
            setAudioParams(data.terminal, data.type, data.num);
            playAudio(audioParams);
          } else {
            releaseStatus.complete = true;
            playAudio(audioParams);
