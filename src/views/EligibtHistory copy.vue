    ...
    <el-row class="table-wrap" :gutter="20">
      <el-col :span="12">
        <el-card>
          ...
          <el-table v-loading="loadingStatus.T1Table" :data="filterTableData[0]" :stripe="false" height="680px"
            :row-class-name="tableRowClassName">
            <el-table-column prop="CAR_ID" label="车牌号" width="130px" />
            <el-table-column prop="TERMINAL_OUT_TIME" label="离开航站楼时间" />
            <el-table-column label="资格获得方式" width="130px">
              <template #default="scope">
                <el-text :type="scope.row.PD == 'system' ? 'info' : 'warnging'">{{ scope.row.PD == "system" ? "系统" :
      "人工" }}</el-text>
              </template>
            </el-table-column>
            <el-table-column label="资格状态">
              <template #default="scope">
      ...
      <el-col :span="12">
        <el-card>
          ...
          <el-table v-loading="loadingStatus.T1Table" :data="filterTableData[1]" :stripe="false" height="680px"
            :row-class-name="tableRowClassName">
            <el-table-column prop="CAR_ID" label="车牌号" width="130px" />
            <el-table-column prop="TERMINAL_OUT_TIME" label="离开航站楼时间" />
            <el-table-column label="资格获得方式" width="130px">
...
async function init() {
  await axios.get("http://10.35.240.224/api/pd_status_history").then((res) => {
