<template>
  <div class="container">
        ...
        <el-card>
          <div style="text-align: center">
            <el-config-provider :locale="locale">
              <el-input :disabled="!!processid" v-model="carNo" placeholder="请输入车牌" class="input-with-select"
                style="max-width: 480px">
                <template #prepend>
                  <el-date-picker :disabled="!!processid" style="margin: 0 -20px 0 -20px; width: 350px"
          ...
          <div style="
            ...
            <div style=" display: flex; justify-content: space-around;">
              <el-button @click="getData('today')" :disabled="!!processid">查询今日数据</el-button>
              <el-button @click="getData()" :disabled="!!processid">查询历史数据</el-button>
            </div>
            <div class="button-wrap">
              <el-button class="button" type="warning" plain @click="authorize" style="width: 110px"
                :disabled="!!processid">授权</el-button>
            </div>
          </div>
        </el-card>
      </el-col>
    ...
    <el-row class="list-box">
      <el-col :span="3"></el-col>
      <el-col :span="8">
        <el-card style="margin-bottom: 20px" v-for="(item, index) in processData">
          <el-row>
            <el-col :span="12">
          ...
          item.RUN_ODOMETER
            ...
            : buffer[index].RUN_ODOMETER
              ...
              <div style="height: 80px;position: relative;">
                <el-button v-show="!processid" :disabled="!buffer[index].TIME_GET_ON"
                ...
                <el-button v-if="item.TIME_GET_ON" @click="Unbinding(item)" style="margin-left: 0"
                  :disabled="!!processid">解绑</el-button>
          ...
          <el-row v-if="processid">
            ...
            <el-col :span="12">
              <div class="input-wrap">
                <el-row>
                  <el-col :span="8"><text>复核时间:</text></el-col>
                  <el-col :span="16"><el-date-picker :disabled-date="disabledDate" style="width: 173px;" size="small"
                      v-model="reviewTime" type="datetime" placeholder="请输入压表时间" /></el-col>
                </el-row>
                <el-row>
                  <el-col :span="8"><text>公里数:</text></el-col>
                  <el-col :span="16"><el-input v-model="mileage" size="small"><template
      ...
      <el-col :span="8" style="position: relative">
        <el-table :data="tableData" style="width: 100%" height="720px">
          ...
          <el-table-column>
            <template #default="scope">
              <el-button v-if="processid" size="small" @click="
        $router.push({
          path: '/showPath',
          query: {
            carNo,
        ...
        ">
              ...
              <el-button v-else size="small" @click="handleBind(scope.row)" :disabled="btnDisabled">
                <el-icon>
                  <CopyDocument />
                </el-icon>
              </el-button>
        ...
        <div v-if="!tableData.length && processid" class="table-mark">
          <el-button @click="gotoShowPath(-1)">查看路径</el-button>
        </div>
      </el-col>
      <el-col :span="3"></el-col>
    </el-row>
    ...
    <div v-if="showReject" class="reject-wrap">
      <div class="reject-box">
        ...
        <div class="reject-button">
          <el-button @click="showReject= !showReject">取消</el-button>
...
import { CopyDocument, MapLocation } from "@element-plus/icons-vue";
...
const reviewTime = ref(dayjs().format("YYYY-MM-DD HH:mm:ss"));
const mileage = ref(0);
const note = ref('');
...
const audit = async (state) => {
  let params = {
    ...
    time_get_on: reviewTime.value,
    judge_runodometer: Number(mileage.value),
    judged: state,
    note: selectValue.value ? selectValue.value : note.value
  };
...
const gotoShowPath = (state, index) => {
  let dateTime;
  if (state > 0) {
    dateTime = [
      buffer[index].TIME_GET_ON,
    ...
    dateTime = [
      dayjs(processData.value[0].POOL_OUT_TIME).add(-2, "hour").format("YYYY-MM-DD HH:mm:ss"),
      dayjs(processData.value[0].POOL_OUT_TIME).add(2, "hour").format("YYYY-MM-DD HH:mm:ss"),
    ];
  }
...
const getData = async (tag) => {
  ...
  await axios.get("/user_list", { params }).then((res) => {
...
onActivated(async () => {
  ...
  if (
    ...
    imgSrc.value = params.imgSrc;
    console.log(params.processid);
    dateTime.value = ['', ''];

    params = {
      processid: processid.value
    };
    await axios.get("/api/pd_judge_process", { params }).then(async res => {
      console.log(res.data);
      processData.value = res.data;
